...
class AgentState(TypedDict, total=False):
    ...
    # Retry-loop gate: routing reads only the latest evaluation, so overwrite (no reducer).
    evaluation_result: str | None
    # Clarification / risky-action / approval: each routing function reads the current
    # decision only, not a history of them, so these overwrite too.
